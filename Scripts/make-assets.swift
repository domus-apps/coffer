...
let groundTop: UInt32 = 0x4FE3A2
let groundBottom: UInt32 = 0x0FA25F
...
// ghost — kept, not gone.
/* Nudged 14px above geometric center: the box carries the glyph's visual
   weight at the bottom, so true centering reads as sitting low. */
let boxBody = CGRect(x: 252, y: 254, width: 520, height: 300)
let card = CGRect(x: 352, y: 434, width: 320, height: 350)
